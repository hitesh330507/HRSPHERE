...
public class DepartmentLifecycleIntegrationTest extends BaseIntegrationTest {
  ...
  void createDepartment_fullLifecycle() {
    ...
    ResponseEntity<DepartmentResponse> createResp =
        restTemplate.postForEntity("/department", createEntity, DepartmentResponse.class);
    ...
    ResponseEntity<ApiErrorResponse> duplicateResp =
        restTemplate.postForEntity("/department", createEntity, ApiErrorResponse.class);
    ...
    // Step 4 — Get by code
    ResponseEntity<DepartmentResponse> getByCodeResp =
        restTemplate.exchange(
            "/department/code/" + created.departmentCode,
            HttpMethod.GET,
            new HttpEntity<>(validatedHdrs),
            DepartmentResponse.class);
    ...
    ResponseEntity<DepartmentResponse> updateResp =
        restTemplate.exchange(
  ...
  void createDepartment_roleEnforcement() {
    ...
    ResponseEntity<ApiErrorResponse> createResp =
        restTemplate.postForEntity("/department", empEntity, ApiErrorResponse.class);
    assertThat(createResp.getStatusCode()).isEqualTo(HttpStatus.FORBIDDEN);

    // ROLE_HR can create
    ResponseEntity<DepartmentResponse> hrCreateResp =
    ...
    ResponseEntity<Void> adminDeleteResp =
        restTemplate.exchange(
            "/department/" + deptId,
            HttpMethod.DELETE,
            new HttpEntity<>(adminHeaders()),
  ...
  void getDepartment_employeeServiceDown_proceedsGracefully() {
    CreateDepartmentRequest createReq =
        DepartmentTestFixtures.validCreateRequest("Resilience Dept");
    ResponseEntity<DepartmentResponse> createResp =
        restTemplate.postForEntity(
    ...
    employeeServiceMock.stubFor(
        get(urlEqualTo("/employees/list?departmentId=" + id + "&size=1"))
            .willReturn(
                aResponse()
                    .withFault(
    ...
    ResponseEntity<DepartmentResponse> getResp =
        restTemplate.exchange(
            "/department/" + id,
            HttpMethod.GET,
            new HttpEntity<>(validatedHdrs),
